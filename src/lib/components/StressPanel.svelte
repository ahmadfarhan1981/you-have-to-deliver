<script lang="ts">
    ...
    $: allStress = $stressSnapshots
    $: stress = allStress.find((s)=>s.person_id===$personStore.person_id)
...
<div class="border border-slate-200 rounded-lg p-6 bg-white shadow-sm mb-6">
    ...
    <div class="grid grid-cols-2 gap-6">
        <div>
            <h4 class="font-bold mb-2 text-slate-700">Stress</h4>
            <div class="space-y-2 text-sm">
                {JSON.stringify(stress)}
                <span class="text-slate-500">Talent:</span> STRESS {stress.current}
